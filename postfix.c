...
*/
	...
	while(ta[i].type != EoS){
		if(ta[i].type == operand){
			...
			if (ta[i].op == '~') {
				...
				eTree* p = create_node(ta[i], NULL, val1);
				push_ps(&nodeStack, p);
			}
			else{
				...
				if(val1 == NULL || val2 == NULL) 
					break;
				eTree* p = create_node(ta[i], val1, val2);
				push_ps(&nodeStack, p);
			}
		}
		i++;
	...
	eTree* result = pop_ps(&nodeStack);
	if(	 result == NULL || !isempty_is(&nodeStack)) {
		fprintf(stderr, "Error in postfix_eval: Invalid postfix expression\n");
		return NULL;
	}
