...
/* Task 2: Implement a function that takes as input (1) a pointer to a partially sorted linked list (intial portion of the list is sorted), (2) data, and (3) an index 
indicating the position till which the list is sorted. It inserts the data in the appropriate position of the sorted initial part. 
*/
...
*/	
	...
	while(temp != NULL && (count < sorted_till+1) && (temp->data < new->data)){
		prev = temp;
		temp = temp->next;
		count++;
	}
...
//Task 3. Solution
	...
	// part->next = list;
	// int p = get_next(part);
	// temp = compare_and_insert(temp,p,0);
	while(temp->next != NULL){
		sLL* temp1 = temp->next;

		int p = get_next(temp);
		list = compare_and_insert(list,p,count);
		if(temp1 == temp->next){
			temp = temp->next;
			count++;
			continue;
