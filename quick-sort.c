...
/* Takes as input an array and two boundary indices.
 Partitions the sub-array given by the boundary indices based on a pivot and 
returns the partition index. That is, all the elements from the left boundary 
to the partition index are less than or equal to the pivot,
 and all the elements from (partition index+1)-th location to the right boundary
  are greater than pivot. This pivot should be generated by calling the 
  get_pivot() function.*/
